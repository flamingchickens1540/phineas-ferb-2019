...
public class QuickTurn extends PIDCommand {

    public static final Logger logger = Logger.getLogger(QuickTurn.class);
    ...
    public QuickTurn(double turnAmount) {
    ...
    protected void initialize() {
        double initialTarget = (Hardware.navx.getYawRadians() - turnAmount);

        relativeGoal = initialTarget - Math.floor(initialTarget / (2 * Math.PI)) * 2 * Math.PI;
    ...
    protected boolean isFinished() {
        ...
        boolean isFinished =
            anglePosError < GOAL_TOLERANCE_ANGULAR_POSITION && angleVelError < GOAL_TOLERANCE_ANGULAR_VELOCITY;
        if (isFinished) {
            logger.debug(String
                .format("Goal reached! Angle error remaining: %f Angular velocity error remaining: %f", anglePosError,
                    angleVelError));
        }
        return isFinished;
    }
